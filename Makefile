...
.PHONY: run-dev
run-dev:
	 flutter run --flavor production --dart-define=FLAVOR=production --target lib/main.dart
...
build-android-prd:
	 flutter build appbundle --no-shrink --no-tree-shake-icons --release --flavor production --dart-define=FLAVOR=production --target lib/main.dart
