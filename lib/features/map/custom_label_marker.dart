...
extension AddExtension on Set<Marker> {
  ...
  ///
  /// * Pass the [CustomLabelMarker] object to add to the set
  /// * !!! IMPORTANT!!!
  ...
  ///          setState(() {});
  ///      });
  Future<bool> addCustomLabelMarker(CustomLabelMarker labelMarker) async {
    var result = false;
...
Future<BitmapDescriptor> createCustomMarkerBitmap(
  ...
  painter.layout();
  painter.paint(canvas, const Offset(20, 10));
  final textWidth = painter.width.toInt();
  final textHeight = painter.height.toInt();
  canvas.drawRRect(
  ...
  final pngBytes = await (await p.toImage(
    painter.width.toInt() + 40,
    painter.height.toInt() + 50,
  ))
