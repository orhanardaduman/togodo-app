...
// ignore_for_file: prefer_asserts_with_message, parameter_assignments, deprecated_member_use
    ...
    if (item.label != null) {
      ...
      text = MediaQuery(
        data: mediaQueryData.copyWith(
          textScaler:
              TextScaler.linear(math.min(1, mediaQueryData.textScaleFactor)),
        ),
        child: text,
      );
    }
...
class _BottomNavigationBarState extends State<CustomBottomNavigationBar>
  ...
  BottomNavigationBarType get _effectiveType {
    if (widget.type != null) {
      return widget.type!;
    } else if (BottomNavigationBarTheme.of(context).type != null) {
      return BottomNavigationBarTheme.of(context).type!;
    } else {
      return widget.items.length <= 3
  ...
  Widget build(BuildContext context) {
    ...
    final bottomTheme = BottomNavigationBarTheme.of(context);
    const layout = BottomNavigationBarLandscapeLayout.spread;
  ...
  Widget build(BuildContext context) {
    ...
    return ClipRRect(
      borderRadius: const BorderRadius.only(
        topLeft: Radius.circular(24), // Sol üst köşe yuvarlatma
        topRight: Radius.circular(24), // Sağ üst köşe yuvarlatma
      ),
